...
ma1 = tm_shape(nz) + tm_fill(fill = "red")
ma2 = tm_shape(nz) + tm_fill(fill = "red", fill_alpha = 0.3)
ma3 = tm_shape(nz) + tm_borders(col = "blue")
ma4 = tm_shape(nz) + tm_borders(lwd = 3)
ma5 = tm_shape(nz) + tm_borders(lty = 2)
ma6 = tm_shape(nz) + tm_fill(fill = "red", fill_alpha = 0.3) +
...
map_nza = tm_shape(nz) +
  tm_fill(fill = "Land_area", fill.legend = tm_legend(title = "legend_title")) + tm_borders()
```
...
tm_shape(nz) + tm_polygons(fill = "Median_income", fill.scale = breaks)
tm_shape(nz) + tm_polygons(fill = "Median_income", fill.scale = 10)
tm_shape(nz) + tm_polygons(fill = "Median_income", palette = "BuGn")
```
...
many_palette_plotter = function(color_names, n, titles){
  ...
  for(i in seq_len(n_colors)){
    if (color_names[i] %in% hcl.pals()){
         one_color = hcl.colors(n = n, palette = color_names[i])
    } else if (color_names[i] %in% palette.pals()){
         one_color = palette.colors(n = n, palette = color_names[i])
    }
...
many_palette_plotter(c("PRGn", "YlGn", "Set 2"), 7, 
...
nz_height_map = tm_shape(nz_elev, bbox = nz_region) +
  tm_raster(col = tm_scale_continuous(values = "YlGn"),
            col.scale = "cont") +
...
- `pages = "year"` is used instead of `by = "year"`.
